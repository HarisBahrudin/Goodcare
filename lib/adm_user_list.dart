...
import 'package:goodcare/adm_user_add.dart';
import 'package:goodcare/adm_user_edit.dart';
...
class AdmUserList extends StatefulWidget {
  final String xtkn;
  const AdmUserList({ Key? key, required this.xtkn }) : super(key: key);

  @override
  _AdmUserListState createState() => _AdmUserListState();
}

class ModelPasien{
  final String id, nm, jk, alamat, tlp, role, email;
  ModelPasien(this.id, this.nm, this.jk, this.alamat, this.tlp, this.email, this.role);
}
...
class _AdmUserListState extends State<AdmUserList> {
  ...
  Future<void> getUser() async {
    ...
    list.clear();
    try {
      final respon = await http.get(Uri.parse(MyApi.apiUserList), headers: {'Authorization':widget.xtkn});
      if (respon.statusCode==200){
          final data = jsonDecode(respon.body);
          data.forEach((a) {
            final mp = ModelPasien(a['id'], a['nm'], a['jk'], a['alamat'], a['tlp'], a['email'], a['role']);
            list.add(mp);
          });
    ...
    return Scaffold(
              ...
              child: Container(
                ...
                Center(child: CircularProgressIndicator()) :
                  ListTile(
                    ...
                    subtitle: Text(x.role=="Kasir" ? "kasir".tr() : x.role, style: TextStyle(fontSize: 18.0)),
                    onTap: (){
      ...
      floatingActionButton: Padding(
        ...
        child: FloatingActionButton(
          ...
          onPressed: (){
            Navigator.of(context).push(MaterialPageRoute(
              builder: (context) => AdmUserAdd(xreload: getUser, xtkn: widget.xtkn)
            ));
          }
        ),
      ),
