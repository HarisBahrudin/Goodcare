...
class AdmPasienAdd extends StatefulWidget {
  final VoidCallback xreload;
  final String xtkn;
  const AdmPasienAdd({ Key? key, required this.xreload, required this.xtkn }) : super(key: key);

  @override
  _AdmPasienAddState createState() => _AdmPasienAddState();
}

class _AdmPasienAddState extends State<AdmPasienAdd> {
  bool loading=false;
  List <ModelKec> list = [];
  ...
  void savePasien() async {
    ...
    try {
      final respon = await http.post(Uri.parse(MyApi.apiPasienAdd), body: {'by':by, 'nik':nik, 'nm':nm, 'jk':jk, 'alamat':alamat, 'kec':xidkec, 'tlp':tlp, 'umur':umur, 'gd':gd}, headers: {'Authorization':widget.xtkn});
