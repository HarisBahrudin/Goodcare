...
class AdmRekapJP extends StatefulWidget {
  final String xtkn;
  const AdmRekapJP({ Key? key, required this.xtkn }) : super(key: key);

  @override
  _AdmRekapJPState createState() => _AdmRekapJPState();
}
...
class _AdmRekapJPState extends State<AdmRekapJP> {
  bool loading = false;
  ...
  void getRekap() async {
    ...
    list.clear();
    try {
      final respon = await http.get(Uri.parse(MyApi.apiRMRekap2), headers: {'Authorization':widget.xtkn});
      if (respon.statusCode==200){
          final data = jsonDecode(respon.body);
          data.forEach((a) {
            final mp = ModelRekap(a['jp'], a['jml']);
            list.add(mp);
          });
