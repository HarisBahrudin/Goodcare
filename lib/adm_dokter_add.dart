...
class AdmDokterAdd extends StatefulWidget {
  final VoidCallback xreload;
  final String xtkn;
  const AdmDokterAdd({ Key? key, required this.xreload, required this.xtkn }) : super(key: key);

  @override
  _AdmDokterAddState createState() => _AdmDokterAddState();
}

class _AdmDokterAddState extends State<AdmDokterAdd> {
  ...
  void saveUser() async {
    ...
    try {
      final respon = await http.post(Uri.parse(MyApi.apiUserAdd), body: {'nm':nm, 'jk':jk, 'alamat':alamat, 'tlp':tlp, 'email':email, 'pwd':pwd, 'role':'Dokter'}, headers: {'Authorization':widget.xtkn});
