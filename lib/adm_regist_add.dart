...
class AdmRegistAdd extends StatefulWidget {
  final VoidCallback xreloadrm;
  final String xtkn;
  const AdmRegistAdd({ Key? key, required this.xreloadrm, required this.xtkn }) : super(key: key);

  @override
  _AdmRegistAddState createState() => _AdmRegistAddState();
}

class ModelDokter{
  final String id, nm;
  ModelDokter(this.id, this.nm);
}

class _AdmRegistAddState extends State<AdmRegistAdd> {
  bool loading = false;
  ...
  void getNoReg() async {
    ...
    try {
      final respon = await http.get(Uri.parse(MyApi.apiRMRegNmr), headers: {'Authorization':widget.xtkn});
  ...
  void saveReg() async {
    ...
    try {
      final respon = await http.post(Uri.parse(MyApi.apiRMRegAdd), body: {'noreg':ctrNoReg.text, 'tgl':ctrTgl.text, 'idpas':idpas, 'iddok':iddok, 'keluhan':keluhan}, headers: {'Authorization':widget.xtkn});
      final data = jsonDecode(respon.body);
      if (respon.statusCode==200){
        if (data['code']=='200'){
            ...
            child: Padding(
              ...
              child: ListView(
                children: [
                  ...
                  //       style: ButtonStyle(backgroundColor: MaterialStateProperty.all(Colors.purple.shade800)),
                  //       child: Row(
                  //         mainAxisAlignment: MainAxisAlignment.center,
                  //         children: [
                  //           Icon(Icons.save, color: Colors.white),
